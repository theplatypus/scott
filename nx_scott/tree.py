...
from .node import MirrorNode, VirtualNode, Node
...
class Tree:
    ...
    def _get_label(self):
        if isinstance(self.root, MirrorNode):
            return self.root.label + "#" + str(self.root.arity) + "{" + str(self.root.magnet) + "}"
        if isinstance(self.root, VirtualNode):
    ...
    def get_order_sequence(self, prop=None):
        self.score_tree()
        lbl = self.root.id if prop is None else self.root.meta.get(prop)
        if self.is_leaf():
            return lbl
        seq = [
            lbl,
