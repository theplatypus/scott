...
from .node import Node, VirtualNode, MirrorNode
...
def from_networkx(nx_graph: 'nx.Graph') -> 'Graph':
    """Build a Graph wrapper from a networkx graph.

    Expected node attributes: label (str).
    Expected edge attributes: modality (str or int).
    """
    graph = Graph(id=getattr(nx_graph, 'name', None))
    node_map = {}
    for node_id, data in nx_graph.nodes(data=True):
        node_label = data.get('label', str(node_id))
        node = Node(str(node_id), node_label)
        graph.add_node(node)
        node_map[node_id] = node

    if nx_graph.is_multigraph():
        edges = nx_graph.edges(keys=True, data=True)
        for idx, (u, v, k, data) in enumerate(edges, start=1):
            modality = data.get('modality', 1)
            edge = Edge(str(idx), node_map[u], node_map[v], modality=modality)
            graph.add_edge(edge)
    else:
        edges = nx_graph.edges(data=True)
        for idx, (u, v, data) in enumerate(edges, start=1):
            modality = data.get('modality', 1)
            edge = Edge(str(idx), node_map[u], node_map[v], modality=modality)
            graph.add_edge(edge)

    graph.meta['nx_node_map'] = {str(k): str(v.id) for k, v in node_map.items()}
    return graph
...
class Graph :
	...
	def fix_cobound(self, cobound_edge_id: str, id_virtual_a: str, id_virtual_b: str) -> bool:
		...
		virtual_node_a = VirtualNode(id_virtual_a, magnet = fingerprint)
		virtual_node_a.meta["floor"] = floor_virtual
		virtual_node_b = VirtualNode(id_virtual_b, magnet = fingerprint)
	...
	def fix_inbound(self, inbound, id_mirror: str, scoring: "TreeScoring" = "$lexic", mode: str = "duplicate") -> bool:
		...
		if mode == "duplicate" :
			...
			for (i, id_edge) in enumerate(id_edges):
				...
				mirror = MirrorNode(id_n_mirror, arity = arity, label = ".", magnet = magnet)
			...
			for (i, id_edge) in enumerate(id_edges):
				...
				id_n_mirror = id_mirror + "_" + str(i)
				mirror = MirrorNode(id_n_mirror, arity = arity, label = node.label, magnet = magnet)
